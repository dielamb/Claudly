...
**Global craft skill — design-first-dev (LOCAL PATCH — not upstream GSD):**

Before generating any plan for a phase that contains UI work, invoke `Skill(skill="design-first-dev")` ONCE at the start of planning. Detect UI work by ANY of:

1. The phase CONTEXT.md or ROADMAP.md goal mentions any of: `UI`, `hero`, `section`, `component`, `layout`, `page`, `form`, `widget`, `screen`, `view`, `dashboard`, `nav`, `button`, `card`, `modal`, `drawer`, `menu`, `typography`, `animation`, `interaction`
2. The phase's planned `files_modified` will contain any of: `*.html`, `*.css`, `*.tsx`, `*.jsx`, `*.vue`, `*.svelte`, `*.astro`, `*.scss`, `*.sass`, `*.less`
3. The phase has a `UI-SPEC.md` file

After reading design-first-dev guidance, bake its principles into EVERY task's `<acceptance_criteria>`:
- Craft classification (HIGH vs LOW) appears in plan frontmatter or objective
- All states covered (loading, error, empty, success) — each gets its own acceptance criterion where applicable
- Design-first: visual spec precedes code-writing tasks in task ordering
- 8px grid and type scale tokens referenced by name (not raw values) in acceptance criteria

This patch exists because plans that don't encode design-first principles in acceptance_criteria produce shallow UI work no matter how good the executor is. The planner is the earliest patch point in the pipeline.
</project_context>
