...
**Global craft skills (LOCAL PATCH — not upstream GSD):**

These apply to ANY project, not just the current one. They live in `~/.claude/skills/` and are invoked via the Skill tool.

1. **`design-first-dev`** — invoke BEFORE the FIRST code task of each plan whose `files_modified` contains ANY of: `*.html`, `*.css`, `*.tsx`, `*.jsx`, `*.vue`, `*.svelte`, `*.astro`, `*.scss`, `*.sass`, `*.less`.
   - Call: `Skill(skill="design-first-dev")`
   - Read the guidance it returns
   - Apply its principles (craft classification HIGH/LOW, all-states coverage, design-first-before-code) to every subsequent Edit/Write in the plan
   - Invoke ONCE per plan, not per task

2. **`bolder`** — invoke when executing in REVISION mode. Detect revision mode by ANY of:
   - `{plan}-SUMMARY.md` already exists in the phase directory
   - Orchestrator passed `--gaps` or `--revision` flag
   - `git log --oneline` shows prior commits matching `(feat|fix)\({plan-slug}\)` in the current branch
   - The same plan has been executed before in this session
   - Call: `Skill(skill="bolder")`
   - Apply its amplification guidance to the revision — the revision must be MORE visually committed than the original, not a cosmetic patch

These two skills are ADDITIVE: a UI plan in revision mode invokes BOTH — `design-first-dev` as the baseline principles and `bolder` as the revision delta. Non-UI plans skip both.

**Exception:** if the user explicitly says "v1 verbatim port" or "byte-for-byte copy" in CONTEXT.md or plan `<objective>`, SKIP both skills and follow the verbatim directive. Skills do not override explicit user port instructions.
</project_context>
