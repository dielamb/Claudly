...
If no path was given, use `~/Desktop/Labirynt` as the default path. Do not ask the user for a path. Also: always run all bash commands with an explicit absolute path (expand `~` to `__HOME__`), and always write `graphify-out/` relative to the INPUT_PATH, not the current working directory — i.e. `INPUT_PATH/graphify-out/`.
...
Graphify: Resolve base path from hardcoded home config (~/.claude/graphify.json) if project config absent; no manual path hints.
<!-- mega-dream 2026-04-24 -->
