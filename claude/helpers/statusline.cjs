...
function getSettings() {
  ...
  _settingsCache = readJSON(path.join(HOME, '.claude', 'settings.json'))
                || readJSON(path.join(HOME, '.claude', 'settings.local.json'))
...
function getLearningStats() {
  const HOME = process.env.HOME || '';
  const memoryPaths = [
    path.join(HOME, '.swarm', 'memory.db'),
    path.join(HOME, '.claude-flow', 'memory.db'),
    path.join(HOME, '.agentdb', 'memory.db'),
...
function getSecurityStatus() {
  const HOME_SEC = process.env.HOME || '';
  const auditData = readJSON(path.join(HOME_SEC, '.claude-flow', 'security', 'audit-status.json'))
...
// Swarm status — reads active-agents.json written by SubagentStart/Stop hooks
function getSwarmStatus() {
  const activeFile = path.join(process.env.HOME || '', '.claude', 'active-agents.json');
  try {
    const agents = readJSON(activeFile) || {};
    const cutoff = Date.now() - 30 * 60 * 1000;
    const active = Object.values(agents).filter(a => a.startedAt > cutoff);
    return {
      activeAgents: active.length,
      maxAgents: CONFIG.maxAgents,
      coordinationActive: active.length > 0,
    };
  } catch (_) {}
...
function getAgentDBStats() {
  ...
  // 1. Count real entries from auto-memory-store.json (project-local, then global fallback)
  const globalClaudeFlow = path.join(os.homedir(), '.claude-flow', 'data');
  const storePaths = [
  ...
  const HOME_HNSW = process.env.HOME || '';
  const hnswPaths = [
    path.join(HOME_HNSW, '.swarm', 'hnsw.index'),
    path.join(HOME_HNSW, '.claude-flow', 'hnsw.index'),
...
function generateStatusline() {
  ...
  // Git branch removed from header — shown on bottom line
  header += '  ' + c.dim + '\u2502' + c.reset + '  ' + c.purple + modelName + c.reset;
  // Duration: skip if > 120min (cumulative API total, misleading)
  const duration = costInfo ? costInfo.duration : session.duration;
  if (duration) {
    const rawMins = parseInt(duration) || 0;
    if (rawMins > 0 && rawMins < 120) header += '  ' + c.dim + '\u2502' + c.reset + '  ' + c.cyan + '\u23F1 ' + duration + c.reset;
  }
  ...
  // RTK token savings
  const rtk = getRTKGain();
  if (rtk) {
    const rtkColor = rtk.avgPct >= 60 ? c.brightGreen : rtk.avgPct >= 30 ? c.brightYellow : c.dim;
    const saved = rtk.savedTokens >= 1000 ? (rtk.savedTokens / 1000).toFixed(1) + 'k' : String(rtk.savedTokens);
    header += '  ' + c.dim + '\u2502' + c.reset + '  ' + rtkColor + '⚡ -' + saved + 't (' + rtk.avgPct + '%)' + c.reset;
  }
  lines.push(header);
  ...
  if (agentdb.hasHnsw && agentdb.vectorCount > 0) {
    ...
    let ruleCount = 0;
    try { const _fs=require('fs'),_p=require('path'),ld=_p.join(process.env.HOME||'','.claude','learning'),sr=(d)=>{_fs.readdirSync(d,{withFileTypes:true}).forEach(f=>{const fp=_p.join(d,f.name);f.isDirectory()?sr(fp):f.name.endsWith('.md')&&(ruleCount+=(_fs.readFileSync(fp,'utf-8').match(/<!-- (?:dream|bulk-dream|mega-dream) /g)||[]).length);})};sr(ld);} catch(_){}
    const label = ruleCount > 0 ? ruleCount + ' rules' : String(progress.patternsLearned) + ' patterns';
    perfIndicator = c.brightYellow + '\uD83D\uDCDA ' + label + c.reset;
  } else {
  ...
  // ── RychuOS / Personal Workflow Line ──────────────────────────────
  try {
    const home = os.homedir();

    // GSD active phase (read from .planning/STATE.md in cwd)
    let gsdPhase = '';
    const stateFiles = [
      path.join(CWD, '.planning', 'STATE.md'),
      path.join(CWD, 'STATE.md'),
    ];
    for (const sf of stateFiles) {
      try {
        if (fs.existsSync(sf)) {
          const st = fs.readFileSync(sf, 'utf-8');
          const m = st.match(/Phase\s+(\d+)|phase[:\s]+(\S+)/i);
          if (m) { gsdPhase = 'GSD:' + (m[1] || m[2]); break; }
          const active = st.match(/active[:\s]+(.+)/i);
          if (active) { gsdPhase = 'GSD:' + active[1].trim().slice(0, 10); break; }
        }
      } catch { /* */ }
    }
    ...
    // Dream Worker — total rule count across all learning files
    let dreamInfo = '';
    try {
      const learningDir = path.join(home, '.claude', 'learning');
      let ruleCount = 0;
      const scanDir = (dir) => {
        try {
          for (const f of fs.readdirSync(dir, { withFileTypes: true })) {
            const fp = path.join(dir, f.name);
            if (f.isDirectory()) scanDir(fp);
            else if (f.name.endsWith('.md')) {
              const txt = fs.readFileSync(fp, 'utf-8');
              ruleCount += (txt.match(/<!-- (?:dream|bulk-dream|mega-dream) /g) || []).length;
            }
          }
        } catch {}
      };
      ...
      if (true) {
        if (ruleCount > 0) {
          dreamInfo = c.dim + 'rules:' + c.reset + c.purple + ruleCount + c.reset;
        }
      }
    } catch { /* */ }

    // Domain from most recent git commit across known projects (last 7 days)
    let domainInfo = '';
    try {
      const knownProjects = [
        { path: path.join(home, 'Desktop', 'Portfolio', 'www_v2'), label: 'portfolio' },
        { path: path.join(home, 'Desktop', 'Jobs', 'career-ops'),  label: 'career-ops' },
        { path: path.join(home, 'RychuOS'),                         label: 'rychuos' },
      ];
      let bestProject = null;
      let bestTs = 0;
      for (const proj of knownProjects) {
        if (!fs.existsSync(proj.path)) continue;
        const ts = parseInt(safeExec(`git -C "${proj.path}" log -1 --format="%at" 2>/dev/null`, 600)) * 1000;
        if (ts && ts > bestTs) { bestTs = ts; bestProject = proj.label; }
      }
      if (bestProject && bestTs > Date.now() - 7 * 24 * 3600 * 1000) {
        domainInfo = c.dim + 'domain:' + c.reset + c.cyan + bestProject + c.reset;
      }
    } catch { /* */ }

    // Server health — only the useful ones
    const checkPort = (port) => {
      try { execSync(`lsof -ti:${port}`, { timeout: 200, stdio: 'pipe' }); return true; } catch { return false; }
    };
    const servers = [
      { port: 8741, label: 'Rychu' },
      { port: 8734, label: '8734' },
    ];
    const serverBits = servers.map(s => {
      const up = checkPort(s.port);
      return (up ? c.brightGreen : c.dim) + s.label + (up ? '✓' : '✗') + c.reset;
    }).join(' ');

    // Assemble line
    const parts = [];
    if (gsdPhase) parts.push(c.brightBlue + gsdPhase + c.reset);
    if (ganScore) parts.push(ganScore);
    if (dreamInfo) parts.push(dreamInfo);
    parts.push(serverBits);

    if (parts.length > 0) {
      lines.push(c.dim + '─'.repeat(53) + c.reset);
      lines.push(c.dim + '⚡ ' + c.reset + parts.join(c.dim + '  │  ' + c.reset));
    }
  } catch { /* never crash statusline */ }
...
// RTK token savings (calls rtk gain -f json)
function getRTKGain() {
  try {
    const raw = safeExec('rtk gain -f json 2>/dev/null', 2000);
    if (!raw) return null;
    const data = JSON.parse(raw);
    const s = data && data.summary;
    if (!s || s.total_commands === 0) return null;
    return {
      savedTokens: s.total_saved || 0,
      avgPct: Math.round(s.avg_savings_pct || 0),
      totalCmds: s.total_commands || 0,
    };
  } catch { return null; }
}
