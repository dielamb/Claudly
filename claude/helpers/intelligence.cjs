...
const DATA_DIR = path.join(process.env.HOME || '', '.claude-flow', 'data');
const STORE_PATH = path.join(DATA_DIR, 'auto-memory-store.json');
const GRAPH_PATH = path.join(DATA_DIR, 'graph-state.json');
const RANKED_PATH = path.join(DATA_DIR, 'ranked-context.json');
const PENDING_PATH = path.join(DATA_DIR, 'pending-insights.jsonl');
const SESSION_DIR = path.join(process.env.HOME || '', '.claude-flow', 'sessions');
const SESSION_FILE = path.join(SESSION_DIR, 'current.json');

// Obsidian vault paths for write-back
const VAULT_PROBLEMS = path.join(process.env.HOME || '', 'Desktop', 'Labirynt', '3 Atlas', 'Problems');

// Router history for learning
const ROUTER_HISTORY_PATH = path.join(DATA_DIR, 'router-history.json');

// ── Inverted trigram index for O(1) lookup at scale ────────────────────────
// Maps trigram → Set of entry IDs. Built during init(), queried during getContext().
let invertedIndex = null; // { trigram: [entryId, ...] }
...
function buildInvertedIndex(rankedEntries) {
  const idx = {};
  for (const entry of rankedEntries) {
    const tris = trigrams(entry.words || []);
    for (const tri of tris) {
      if (!idx[tri]) idx[tri] = [];
      idx[tri].push(entry.id);
    }
  }
  return idx;
}

function queryInvertedIndex(promptTrigrams, topK) {
  if (!invertedIndex) return null;
  // Count how many prompt trigrams each entry matches
  const hits = {};
  for (const tri of promptTrigrams) {
    const ids = invertedIndex[tri];
    if (ids) {
      for (const id of ids) {
        hits[id] = (hits[id] || 0) + 1;
      }
  ...
  // Return top candidates by hit count (pre-filter before full Jaccard)
...
function writeInsightToObsidian(summary, content) {
  try {
    if (!fs.existsSync(VAULT_PROBLEMS)) return false;
    const date = new Date().toISOString().split('T')[0];
    const slug = summary.replace(/[^a-z0-9]+/gi, '-').toLowerCase().slice(0, 50);
    const filename = `${date}-${slug}.md`;
    const filepath = path.join(VAULT_PROBLEMS, filename);
    ...
    const md = [
      ...
      `title: "${summary}"`,
      `date: ${date}`,
      'source: intelligence-consolidation',
      'tags: [auto-generated, hot-path]',
      '---',
      '',
      `# ${summary}`,
      '',
      content,
      '',
      `> Auto-generated by intelligence.cjs on ${date}`,
    ].join('\n');
...
function init() {
  ...
  // Build inverted trigram index for O(1) lookup at scale
  invertedIndex = buildInvertedIndex(rankedEntries);
...
function getContext(prompt) {
  ...
  const TOP_K = 5;
  const PREFILTER_K = 50; // max candidates from inverted index

  // Pre-filter using inverted index if available (O(1) per trigram vs O(n) full scan)
  let candidates = ranked.entries;
  if (invertedIndex && ranked.entries.length > PREFILTER_K) {
    const candidateIds = queryInvertedIndex(promptTrigrams, PREFILTER_K);
    if (candidateIds && candidateIds.length > 0) {
      const idSet = new Set(candidateIds);
      candidates = ranked.entries.filter(e => idSet.has(e.id));
    }
  }
...
/**
 * negativeFeedback(prompt) — Called from route when negative signals detected.
 * Decays confidence for last-matched patterns more aggressively than normal decay.
 */
function negativeFeedback(prompt) {
  const matchedIds = sessionGet('lastMatchedPatterns');
  if (!matchedIds || !Array.isArray(matchedIds)) return;
  boostConfidence(matchedIds, -0.08); // stronger than normal -0.02
}
...
function consolidate() {
  ...
  if (fs.existsSync(PENDING_PATH)) {
    ...
    // Write new insights to Obsidian (permanent memory)
    for (const [file, count] of Object.entries(editCounts)) {
      if (count >= 3) {
        const basename = path.basename(file);
        writeInsightToObsidian(
          `Frequently edited: ${basename}`,
          `File \`${file}\` was edited ${count} times in a single session.\n\n` +
          `This suggests it's a hot path — consider:\n` +
          `- Adding tests if not covered\n` +
          `- Reviewing for refactoring opportunities\n` +
  ...
  // 8.5. Rebuild inverted index with new entries
  invertedIndex = buildInvertedIndex(rankedEntries);
...
module.exports = { init, getContext, recordEdit, feedback, negativeFeedback, detectNegativeSignal, consolidate, stats };
